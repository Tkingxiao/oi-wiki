...
export async function getAlbumsWithLatestPhotos() {
    try {
        ...
        for (const album of albums) {
            ...
            let img = '';
            if (latestPhoto && fileExists(latestPhoto.url)) img = `/api/file/${latestPhoto.url}`;
        ...
        `);
        const processedLatestPhotos = latestPhotos.filter(photo => fileExists(photo.url)).map(photo => ({
            id: photo.id, photoAlbumId: photo.album_id, title: photo.name, img: `/api/file/${photo.url}`
        }));
...
export async function getPhotos(albumId) {
    try {
        ...
        const photos = queryAll(`SELECT p.id, p.name, p.url FROM photo p WHERE p.album_id = ? AND p.is_deleted = 0 AND p.is_review = 1 ORDER BY p.create_time DESC`, [albumId]);
        const processedPhotos = photos.filter(photo => fileExists(photo.url)).map(photo => ({ id: photo.id, name: photo.name, url: `/api/file/${photo.url}` }));
...
export async function createAlbum(albumData, userId, userPermission) {
    ...
    try {
        ...
        if (name.length > 100) return { success: false, message: '相册名称不能超过100个字符' };
        if (introduction && introduction.length > 500) return { success: false, message: '相册简介不能超过500个字符' };
...
export async function uploadPhoto(file, photoData, userId, userPermission, albumInfo, uploadDir) {
    ...
    try {
        if (!albumId) return { success: false, message: '相册ID不能为空' };
        ...
        const currentTime = Math.floor(Date.now() / 1000);
        const result = insert(`INSERT INTO photo (album_id, user_id, name, url, is_review, create_time, update_time) VALUES (?, ?, ?, ?, ?, ?, ?)`, [albumId, userId, name.trim(), filePath, isReview, currentTime, currentTime]);
        ...
        return { success: true, message, data: { photoId: result.lastInsertRowid, name: name.trim(), url: filePath, album: album.name, isReview } };
    } catch (error) {
...
export async function getAlbumsForAdmin() {
    try {
        ...
        for (const album of albums) {
            ...
            const processedPhotos = photos.map(photo => ({ id: photo.id, album_id: photo.album_id, user_id: photo.user_id, user_name: photo.user_name, name: photo.name, url: `/api/file/${photo.url}`, is_review: photo.is_review, create_time: photo.create_time, update_time: photo.update_time }));
